...
local function _load_resp_headers( self )
    ...
    while true do
        ...
        line, err_msg = _read_line( self )
        if err_msg ~= nil then
            return 'SocketError', 'read header:' .. err_msg
        end
        ...
        if table.getn(elems) < 2 then
            return 'BadHeader', 'invalid header:' .. line
        end
    ...
    if cont_len ~= nil then
        ...
        if cont_len == nil then
            return 'BadHeader', 'invalid content-length header:'
                    .. tostring(self.headers['content-length'])
        end
...
local function _norm_headers( headers )
    ...
    for h, v in pairs( headers ) do
        ...
        for _, header_val in ipairs( v ) do
            table.insert( hs, string.format('%s: %s', h, header_val ) )
        end
    end
...
local function _read_chunk_size( self )
    ...
    if err_msg ~= nil then
        return nil, 'SocketError', 'read chunk size:' .. err_msg
    end
    ...
    if size == nil then
        return nil, 'BadChunkCoding', 'invalid chunk size:' .. line
    end
...
local function _next_chunk( self )
    ...
    if size == 0 then
        ...
        if err_code ~= nil then
            return err_code, 'read trailer:' .. err_msg
        end
    end
...
local function _read_chunk( self, size )
    ...
    while size > 0 do
        ...
        if err_msg ~= nil then
            return nil, 'SocketError', 'read chunked:' .. err_msg
        end
...
function _M.send_request( self, uri, opts )
    ...
    local sbuf = {string.format( '%s %s HTTP/1.1', self.method, self.uri ),
    ...
    if err_msg ~= nil then
        return 'SocketError', 'connect:' .. err_msg
    end

    ret, err_msg = self.sock:send( sbuf )
    if err_msg ~= nil then
        return 'SocketError', 'request:' ..  err_msg
    end
...
function _M.send_body( self, body )
    ...
    if body ~= nil then
        ...
        if err_msg ~= nil then
            return nil, 'SocketError', 'send body:' .. err_msg
        end
    end
...
function _M.read_body( self, size )
    ...
    if err_msg ~= nil then
        return nil, 'SocketError', 'read body:' .. err_msg
    end
...
function _M.set_keepalive( self, timeout, size )
    ...
    if err_msg ~= nil then
        return 'SocketError', 'set keepalive:' .. err_msg
    end
